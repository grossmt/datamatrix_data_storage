...
def build_packet(packet: Union[StateControlRequest, SettingsSetRequest, ArchieveDataResponse]) -> bytes:
    ...
    b_packet_code = packet.packet_code.to_bytes(
        PACKET_CODE_LEN, byteorder=BYTEORDER
    )
    ...
    if isinstance(packet, StateControlRequest):
        b_reserved = packet.reserved.to_bytes(STATE_CONTROL_RESERVED_LEN, byteorder=BYTEORDER)
        ...
        b_reserved = packet.settings.reserved.to_bytes(SETTINGS_RESERVED_LEN, byteorder=BYTEORDER)
